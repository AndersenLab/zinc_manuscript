...
# load file s1
dosepruned <- read.csv("data/S1_File.csv")
...
cowplot::ggdraw() +
  cowplot::draw_image("figures/extra/FigS3_scantwo.png")
...
NIL_pruned <- read.csv("data/S9_File.csv")
...
doseprunedV3 <- read.csv("data/S12_File.csv")
...
sqst5_pruned <- read.csv("data/S17_File.csv")
HTA_stats <- read.csv("data/S10_File.csv")
nilgeno <- read.csv("data/S8_File.csv")
...
sqst5_NIL_pruned <- read.csv("data/S18_File.csv")
HTA_stats <- read.csv("data/S10_File.csv")
nilgeno <- read.csv("data/S8_File.csv")
...
trt <- "median.EXT"
tsize <- 12
strainset <- rev(c("N2", "CB4856", "ECA859", "ECA2517", "ECA2518"))
...
```{r figure_s10, fig.height = 5, fig.width = 7.5}
...
**S10 Fig. Geographical distribution of 328 wild isolates.** Map of wild isolates. Strains are colored by the variation haplotype at *sqst-5* (Wild-type: grey, N2: orange, CB4856: navy, Deletion: magenta, Other putative structural variation: light pink). Strains with the deletion are labeled.
...
```{r figure_s11, fig.height = 8, fig.width = 7.5}
...
```{r figure_s12, fig.height = 5, fig.width = 7.5}
...
metal_map <- read.csv("data/S27_File.csv")
