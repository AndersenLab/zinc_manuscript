...
chrV_genes <- query_genes("V:10084029-11345444", "zinc")
chrV_genes_metal <- query_genes("V:10084029-11345444", "metal|zinc|cadmium|iron|copper|magnesium|manganese|cobalt")

# chrV CI
14428285-10084029 # 4.3 Mb

# chrV NIL + CI
11345444-10084029 # 1.26 Mb
...
genes_of_interest <- chrV_genes %>%
    dplyr::filter(go_annotation == T, pc_vars == T)
genes_of_interest2 <- chrV_genes %>%
    dplyr::filter(go_annotation == T, eqtl == T)

# variation in zhit-3 and H27A22.1
zinc_genes <- cegwas2::query_vcf(c("zhit-3", "WBGene00010418"), samples = "CB4856") %>%
    dplyr::filter(a1 == ALT)

# analyze genes on chromosome V with any metal annotation
chrV_metal <- query_genes("V:9620518-11345444", "metal|zinc|cadmium|iron|copper|magnesium|manganese|cobalt")

genes_of_interestm <- chrV_metal %>%
    dplyr::filter(go_annotation == T, pc_vars == T)
genes_of_interestm2 <- chrV_metal %>%
    dplyr::filter(go_annotation == T, eqtl == T)
# VL
# analyze genes on chromosome V with zinc annotation
chrV_genesL <- query_genes("V:9620518-10511995", "zinc")

genes_of_interest <- chrV_genesL %>%
    dplyr::filter(go_annotation == T, pc_vars == T)
genes_of_interest2 <- chrV_genesL %>%
    dplyr::filter(go_annotation == T, eqtl == T)

# analyze genes on chromosome V with any metal annotation
chrV_metal <- query_genes("V:9620518-10511995", "metal|zinc|cadmium|iron|copper|magnesium|manganese|cobalt")


# VR
# analyze genes on chromosome V with zinc annotation
chrV_genesR <- query_genes("V:10511995-11345444", "zinc")

genes_of_interest <- chrV_genesR %>%
    dplyr::filter(go_annotation == T, pc_vars == T)
genes_of_interest2 <- chrV_genesR %>%
    dplyr::filter(go_annotation == T, eqtl == T)

# nhr-103 has wide eqtl ci?
# A_12_P114794
load("~/Dropbox/AndersenLab/LabFolders/Katie/projects/zinc/manuscript/data/processed/FileSX_nhr103_map.Rda")
maxlodplot(nhr103_map)
# maps to divergent region on VL @ 2.271 Mb
...
# chrIII_genes <- query_genes("III:4664-612245", "zinc") # 597553
chrIII_genes <- query_genes("III:4664-597553", "zinc") # 597553

# CI with median.EXT is much smaller!
# chrIII_genes <- query_genes("III:116449-651305", "zinc")

# analyze genes on chromosome III with any metal annotation
chrIII_metal <- query_genes("III:4664-612245", "metal|zinc|cadmium|iron|copper|magnesium|manganese|cobalt")
# chrIII_metal <- query_genes("III:116449-651305", "metal|zinc|cadmium|iron|copper|magnesium|manganese|cobalt")

# 4 genes have PC variation and zinc annotation, 3 additional have a metal annotation and PC variation
genes_of_interest <- chrIII_metal %>%
    dplyr::filter(go_annotation == T, pc_vars == T)
...
chrIII_plot <- df %>%
    ...
    #                dplyr::filter(sample %in% c("ECA2517"), chrom == "III") %>%
    #                dplyr::mutate(chrom = "chrIII") %>%
    #                dplyr::mutate(ystrain%>%,
...
tree <- ape::read.tree("data/S24_File.tree")
...
# ggsave("~/Dropbox/AndersenLab/LabFolders/Katie/projects/zinc/manuscript/figures/Fig7_gwas.png", width = 7.5, height = 8)
ggsave("figures/Fig7_gwas.png", width = 7.5, height = 8)

chrIIIr_genes <- query_genes("III:11251184-11611789", "zinc")
